...
For more details please see the ForeCA paper(s):
```{r cite-ForeCA}
citation("ForeCA")
```
...
The **ForeCA** package achieves this via
  ...
  * the **sapa** and **astsa** package for good non-parametric estimates 
    of the spectrum, and 
...
The `EuStockMarkets` data contains daily closing prices for `r
ncol(EuStockMarkets)` major European stock markets.  As usual we convert this to
